...
import { uploadLancementDocument } from '@/lib/actions/projet-lancement';
...
interface LancementUploadButtonProps {
  projetId: string;
  projetRef: string;
  etapeKey: string;
}

export function LancementUploadButton({
  projetId,
  projetRef,
  etapeKey,
}: LancementUploadButtonProps) {
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const result = await uploadLancementDocument(
        projetId,
        projetRef,
        etapeKey,
        formData,
