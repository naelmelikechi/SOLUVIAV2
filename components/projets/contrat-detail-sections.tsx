...
// Rendu partagé du détail d'un contrat : le Sheet (coup d'oeil depuis la
// table) rend le mode compact (3 sections clés), la page
// /projets/[ref]/contrats/[id] rend tout. Une seule source pour les labels
// Eduvia et la mise en forme.
...
export function ContratDetailSections({
  data,
  compact = false,
}: {
  data: ContratDetail;
  /** true = 3 sections clés (Sheet coup d'oeil) ; false = fiche complète. */
  compact?: boolean;
}) {
  return (
    <div className="space-y-5">
      <Section icon={Hash} title="Identifiants">
        <Row label="DECA / OPCO" value={data.contrat.contract_number} mono />
        <Row label="N° Eduvia" value={data.contrat.internal_number} mono />
      ...
      <Section icon={Calendar} title="Contrat">
        <Row
          label="Type"
          value={mapLabel(CONTRACT_TYPE_LABELS, data.contrat.contract_type)}
        />
        <Row
          label="Mode"
          value={mapLabel(CONTRACT_MODE_LABELS, data.contrat.contract_mode)}
        />
        <Row
          label="Mode de création"
          value={mapLabel(CREATION_MODE_LABELS, data.contrat.creation_mode)}
        />
        <Row
          label="Date conclusion"
          value={fmtDate(data.contrat.contract_conclusion_date)}
        />
        <Row label="Acceptation" value={fmtDate(data.contrat.accepted_at)} />
        <Row
        ...
        <Row
          label="Durée"
          value={
            data.contrat.duree_mois ? `${data.contrat.duree_mois} mois` : null
          }
        />
        <Row
        ...
        Number(data.contrat.support) !== Number(data.contrat.npec_amount) ? (
          <Row
        ...
        Number(data.contrat.support_first_equipment) > 0 ? (
          <Row
            ...
            value={formatCurrency(Number(data.contrat.support_first_equipment))}
            mono
          />
        ) : null}
        <Row
      ...
      {!compact && data.apprenant && (
        <Section icon={User} title="Apprenant">
          ...
          <Row
            label="Statut"
            value={mapLabel(APPRENANT_STATUS_LABELS, data.apprenant.status)}
          />
          <Row
            label="RQTH"
            value={
      ...
      {!compact && data.formation && (
        <Section icon={GraduationCap} title="Formation">
          <Row label="Titre" value={data.formation.qualification_title} />
          <Row label="RNCP" value={data.formation.rncp} mono />
          <Row label="Code diplôme" value={data.formation.code_diploma} mono />
          <Row label="Type diplôme" value={data.formation.diploma_type} />
      ...
      {!compact && data.company && (
        <Section icon={Building2} title="Entreprise d'accueil">
          ...
          />
          <Row label="Type employeur" value={data.company.employer_type} />
          <Row
            label="Adresse"
            value={
              [
      ...
      {!compact && data.progression && (
        <Section icon={TrendingUp} title="Progression pédagogique (Eduvia)">
          <Row
      ...
      {!compact && data.forecastSteps.length > 0 && (
        ...
        <Section icon={Receipt} title="Facturation Eduvia">
          <div className="space-y-1.5">
            {data.invoiceSteps.map((s) => {
              ...
              const isPaid = s.invoice_state === 'REGLE' || s.paid_at !== null;
                ...
                >
                  <div className="flex min-w-0 flex-col">
                    ...
                    <div className="text-muted-foreground flex items-center gap-1.5 text-[11px]">
                      ...
                      {s.external_code && (
                        <span className="font-mono">· {s.external_code}</span>
                      )}
                    </div>
                  </div>
      ...
      <div className="text-muted-foreground pt-2 text-[10px]">
        ...
        {data.contrat.last_synced_at
          ? new Date(data.contrat.last_synced_at).toLocaleString('fr-FR')
          : 'jamais'}
      </div>
    </div>
  );
