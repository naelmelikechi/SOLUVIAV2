...
export async function getClientById(id: string) {
  ...
  const { data, error } = await supabase
    .from('clients')
    .select(
      'id, trigramme, raison_sociale, siret, adresse, localisation, numero_qualiopi, numero_nda, numero_uai, date_entree, archive, tva_intracommunautaire, is_demo, created_at, apporteur_commercial_id, apporteur_date, apporteur:users!clients_apporteur_commercial_id_fkey(id, nom, prenom)',
    )
    .eq('id', id)
    .single();
