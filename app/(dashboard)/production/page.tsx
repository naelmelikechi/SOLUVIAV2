...
function generateMonthlyData(perspective: 'opco' | 'soluvia'): MonthRow[] {
  const commission = perspective === 'soluvia' ? 0.1 : 1;
  const today = new Date();
  const rows: Omit<MonthRow, 'raf' | 'rae'>[] = [];
  ...
  for (let offset = -6; offset <= 6; offset++) {
    ...
    const baseProduction = (12000 + Math.floor(offset * 500)) * commission;
    const baseFacture = isFuture ? 0 : baseProduction * 0.85;
    const baseEncaisse = isFuture ? 0 : baseFacture * 0.9;
    const enRetard = isFuture
      ? 0
      : Math.max(0, baseFacture - baseEncaisse - 1000);

    rows.push({
      date,
      label: capitalize(format(date, 'MMM yyyy', { locale: fr })),
      production: Math.round(baseProduction),
      facture: Math.round(baseFacture),
      encaisse: Math.round(baseEncaisse),
      en_retard: Math.round(enRetard),
      isFuture,
      isCurrent,
    });
  }
...
export default function ProductionPage() {
  ...
  const data = useMemo(() => generateMonthlyData(perspective), [perspective]);
