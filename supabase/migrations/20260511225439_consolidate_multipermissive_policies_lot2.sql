...
-- Consolidation des 8 groupes multi-permissive restants (lot 2 apres
-- e32879c pour projets/clients/contrats). Pattern : pour chaque (table, cmd)
-- avec >1 policy PERMISSIVE, on DROP les sources et on CREATE 1 policy
-- consolidee avec les conditions OR'ed. Les ALL policies sont split en
-- 4 cmd-specifiques pour eviter d'introduire de nouveaux multi-permissive.

-- ---------------------------------------------------------------------------
-- absences : 1 ALL (own) + 2 SELECT (admin + own) -> 4 cmd-specifiques
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- factures : INSERT 2 + SELECT 2 -> consolide. DELETE/UPDATE inchanges.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- idees : 1 ALL admin + 1 INSERT propose + 1 SELECT all + 1 UPDATE author
--        + 2 UPDATE shipper/validator -> split en 4 cmd-specifiques
-- ---------------------------------------------------------------------------
...
CREATE POLICY idees_select ON public.idees
  FOR SELECT USING (true);
CREATE POLICY idees_insert ON public.idees
  FOR INSERT WITH CHECK (
    public.is_admin()
    OR (auteur_id = (SELECT auth.uid()) AND statut = 'proposee'::statut_idee)
  );
CREATE POLICY idees_update ON public.idees
  FOR UPDATE USING (
    public.is_admin()
...
CREATE POLICY idees_delete ON public.idees
  FOR DELETE USING (public.is_admin());

-- ---------------------------------------------------------------------------
-- rdv_commerciaux : 1 ALL admin + 1 ALL pipeline_write + 1 SELECT pipeline_read
-- ---------------------------------------------------------------------------
...
CREATE POLICY rdv_commerciaux_select ON public.rdv_commerciaux
  FOR SELECT USING (
    public.is_admin() OR public.has_pipeline_access()
  );
CREATE POLICY rdv_commerciaux_insert ON public.rdv_commerciaux
  FOR INSERT WITH CHECK (
    public.is_admin()
    OR (public.has_pipeline_access() AND commercial_id = (SELECT auth.uid()))
  );
CREATE POLICY rdv_commerciaux_update ON public.rdv_commerciaux
  FOR UPDATE USING (
    public.is_admin()
    OR (public.has_pipeline_access() AND commercial_id = (SELECT auth.uid()))
  ) WITH CHECK (
...
CREATE POLICY rdv_commerciaux_delete ON public.rdv_commerciaux
  FOR DELETE USING (
    public.is_admin()
    OR (public.has_pipeline_access() AND commercial_id = (SELECT auth.uid()))
  );
...
-- ---------------------------------------------------------------------------
-- rdv_formateurs : 1 ALL admin + 1 ALL cdp_write + 1 SELECT cdp_read
-- ---------------------------------------------------------------------------
...
CREATE POLICY rdv_formateurs_select ON public.rdv_formateurs
  FOR SELECT USING (
    public.is_admin()
    OR EXISTS (
      SELECT 1 FROM projets p
...
CREATE POLICY rdv_formateurs_insert ON public.rdv_formateurs
  FOR INSERT WITH CHECK (
    public.is_admin()
    OR EXISTS (
      SELECT 1 FROM projets p
...
CREATE POLICY rdv_formateurs_update ON public.rdv_formateurs
  FOR UPDATE USING (
    public.is_admin()
    OR EXISTS (
      SELECT 1 FROM projets p
...
CREATE POLICY rdv_formateurs_delete ON public.rdv_formateurs
  FOR DELETE USING (
    public.is_admin()
    OR EXISTS (
      SELECT 1 FROM projets p
...
-- ---------------------------------------------------------------------------
-- saisies_temps : 3 ALL policies (admin, cdp_own, users_write_internes)
-- cdp_own (user_id = uid) domine users_write_temps_internes (user_id = uid
-- AND est_interne). Consolidation simple : admin OR user_id = uid.
-- ---------------------------------------------------------------------------
...
CREATE POLICY saisies_temps_select ON public.saisies_temps
  FOR SELECT USING (
    public.is_admin() OR user_id = (SELECT auth.uid())
  );
CREATE POLICY saisies_temps_insert ON public.saisies_temps
  FOR INSERT WITH CHECK (
    public.is_admin() OR user_id = (SELECT auth.uid())
  );
CREATE POLICY saisies_temps_update ON public.saisies_temps
  FOR UPDATE USING (
    public.is_admin() OR user_id = (SELECT auth.uid())
  ) WITH CHECK (
    public.is_admin() OR user_id = (SELECT auth.uid())
  );
CREATE POLICY saisies_temps_delete ON public.saisies_temps
  FOR DELETE USING (
    public.is_admin() OR user_id = (SELECT auth.uid())
  );

-- ---------------------------------------------------------------------------
-- saisies_temps_axes : 3 ALL policies similaires (via saisies_temps.user_id)
-- ---------------------------------------------------------------------------
...
CREATE POLICY saisies_temps_axes_select ON public.saisies_temps_axes
  FOR SELECT USING (
    public.is_admin()
    OR EXISTS (
...
CREATE POLICY saisies_temps_axes_insert ON public.saisies_temps_axes
  FOR INSERT WITH CHECK (
    public.is_admin()
    OR EXISTS (
      SELECT 1 FROM saisies_temps st
...
CREATE POLICY saisies_temps_axes_update ON public.saisies_temps_axes
  FOR UPDATE USING (
    public.is_admin()
    OR EXISTS (
      SELECT 1 FROM saisies_temps st
...
CREATE POLICY saisies_temps_axes_delete ON public.saisies_temps_axes
  FOR DELETE USING (
    public.is_admin()
    OR EXISTS (
      SELECT 1 FROM saisies_temps st
